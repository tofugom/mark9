...
- [x] Collab 코드 제거 (`apps/server`, `packages/collab`, `App.tsx` 참조 정리)
- [x] `@mark9/comments` 패키지 스켈레톤
- [x] `@mark9/viewer` 패키지 스켈레톤
- [x] 코멘트 UI (선택 → 버블 → 사이드 패널, 마진 배지, 패널 토글)
- [x] 데모 앱 (`apps/web`) 마이그레이션 + LightningFS 사이드카
- [x] 실시간 WYSIWYG 편집 + 자동 저장
- [x] 다크/라이트 토글, 설정 패널 정리
- [x] syntax-hint (커서 위치 기반 마크다운 문법 표시), tab focus trap
